...
public class PanelSide extends JPanel implements Runnable{
	...
	public void paint(Graphics g){
		...
		g.fillRect((int)(Main.renderer.povX * 10),(int)(Main.renderer.povZ * 10), 5, 5);
		for(int i = 0; i < Main.renderer.rawPolygons.size();i++){
			...
			for(int j = 0 ; j < p3.sides.size();j++){
				for(int k = 0; k < p3.sides.get(j).points.size();k++){
					...
					g.fillRect((int)(p.x * 10),(int)(p.z * 10), 5, 5);
				}
			}
		}
	}
